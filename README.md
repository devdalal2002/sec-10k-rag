...
**100% recall@10 with named-entity metadata filtering.** [Full eval results](eval/results.md)
...
not a retrieval issue. See [the failure case](eval/results.md#generation-failure-q20-goldman-sachs-net-earnings-fy2023)
in results.md.
...
**Requirements:** Python 3.10+, Ollama with `qwen2.5:7b` pulled, ~4 GB disk.
...
# Download filings from SEC EDGAR (edit tickers/years in src/download_filings.py)
python src/download_filings.py

# Extract text, chunk, embed -> ChromaDB (both collections)
python src/embed.py

# Full eval run (~11 min with warm rerank cache)
python src/eval.py

# Re-run generation only with stored retrieval results (8.7 min)
python src/eval.py --regen

# Recompute metrics from stored answers without re-generating
python src/eval.py --recompute
```
...
│   └── eval.py                # Harness: 520 retrieval runs + 65 generation runs
├── eval/
│   ├── ground_truth.csv       # 65 hand-verified questions with metadata
│   ├── raw_results.jsonl      # Raw per-(qid, collection, config) results
│   └── results.md             # Summary tables, observations, failure analysis
├── data/
│   ├── chunks/                # JSONL chunk files (recursive + section_aware)
